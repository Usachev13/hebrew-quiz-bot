...
def collect(scope):
    ...
    if scope in ("all", "phrases"):
        ...
        for _sit, item in ph.all_phrases():
            # Перебираем ЯЗЫКИ, а не только формы. У большинства фраз
            # иврит от языка интерфейса не зависит, и повторы отсеются
            # ниже. Но есть исключения (поле he_en): «Кто-нибудь говорит
            # по-русски?» англоязычному произносится про английский, и
            # это другой звуковой файл.
            #
            # Раньше здесь стоял один вызов без языка. Новая фраза
            # добавилась, генератор её не увидел — и приложение искало бы
            # звук, которого нет, молча и без ошибки. Перебор по языкам
            # закрывает это на будущее: любая новая he_en попадёт сюда
            # сама, без правки этого файла.
            for lang in ph.LANGS:
                items.append(ph.spoken(item, female=False, lang=lang))
                if item.get("he_f"):
                    items.append(ph.spoken(item, female=True, lang=lang))
