...
|------|---------|--------|------------|
| Фон | `#F4EFE6` | `#171512` | `--bg` |
| Карточка | `#FFFFFF` | `#232019` | `--surface` |
...
Нейтрали **тёплые**, а не серо-синие. Это и есть разница между
«системная тема» и «бумага с чернилами»: у синеватого угля красного
меньше, чем синего, у тёплого — больше. Замеряется вычитанием каналов
(R − B): должно быть больше нуля.

Золото кладём **волоском по верхней кромке карточек**, а не рамкой:
приём из печати, тонкая линия благородного цвета читается как отделка,
толстая — как дешёвая позолота.
